...
axis on;
% zoom(10)
for amplitudeThreshold = 10 : 0.1 : 15
% Find the location of the big spikes.
% amplitudeThreshold = 11.5;
frequencyImage_prime(:, :) = frequencyImage;
...
brightSpikes(round(rows/2) - 5 : round(rows/2) + 5, round(columns/2) - 5: round(columns/2) + 5) = 0;
imshow(brightSpikes);
...
frequencyImage_prime(brightSpikes) = 0;
% Take log magnitude so we can see it better in the display.
amplitudeImage2 = log(abs(frequencyImage_prime));
...
filteredImage = ifft2(fftshift(frequencyImage_prime));
amplitudeImage3 = abs(filteredImage);
[amplitudeImage3,estDoS] = imnlmfilt(amplitudeImage3);
minValue = min(min(amplitudeImage3));
maxValue = max(max(amplitudeImage3));
subplot(2, 3, 6);
imshow(amplitudeImage3, [minValue maxValue]);
title(sprintf('Filtered Image %s', amplitudeThreshold), 'FontSize', fontSize);
% set(gcf, 'units','normalized','outerposition',[0 0 1 1]); % Maximize figure.
% % 
end